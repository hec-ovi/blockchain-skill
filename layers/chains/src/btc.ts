...
/** Broadcast a raw tx hex. Esplora POST /tx on public networks, bitcoind sendrawtransaction otherwise. */
export async function broadcastBtcTx(info: BtcChainInfo, txHex: string, postFn: PostLike = defaultPost): Promise<string> {
  if (info.esploraUrls.length === 0) {
    return (await bitcoindCall(info, "sendrawtransaction", [txHex])) as string;
  }
  let lastError = "";
  for (const base of info.esploraUrls) {
    try {
